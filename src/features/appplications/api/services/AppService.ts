...
export const AppService = {
  getAll: async (tenantId: string): Promise<AppDto[]> => {
    const response = await api.get<AppDto[]>(`/config/apps/${tenantId}`, {
      headers: {
  ...
  delete: async (tenantId: string, appId: string): Promise<void> => {
    await api.delete<void>(`/config/apps/delete/${tenantId}/${appId}`, {
      headers: {
        "X-Tenant-Id": tenantId,
      },
    });
  ...
  getConsumers: async (
    ...
    const response = await api.get<ConsumerDto[]>(
